<script lang="ts">
	...
	import type { ImageSrc } from './Image.ts';
	...
	const handleImgError = () => {
		...
		const index = imageSrc.fallback.findIndex(
			(url) => new URL(url).toString() === new URL(img.src).toString()
		);
		if (index === -1) {
			fallbackUrl = imageSrc.fallback[0];
		} else {
			fallbackUrl = imageSrc.fallback[index + 1];
		}
...
<img
	...
	src={imageSrc.img}
	srcset={imageSrc.srcsets ? imageSrc.srcsets.map((s) => `${s.src} ${s.w}w`).join(', ') : ''}
	{alt}
	{title}
